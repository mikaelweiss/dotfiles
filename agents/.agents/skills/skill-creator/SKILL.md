---
name: skill-creator
description: Create new Codex skills through guided conversation. Use when the user wants to create a skill, write a skill, make a skill, or set up custom instructions for a specific domain or workflow.
user-invocable: true
---

# Skill Creator

Guide users through creating effective Codex skills via structured questioning. Never assume — always ask.
...
5. **Scope** — Project-specific (`.Codex/skills/`) or global (`~/.Codex/skills/`)?
...
The `description` field is critical — Codex uses it to decide when to apply the skill.
...
model: Codex-sonnet-4-20250514          # Force specific model
...
| Path | Scope |
|------|-------|
| `~/.Codex/skills/` | Personal — all projects |
| `.Codex/skills/` | Project — shared with team |
